...
Classroom desk caddy holding **8 lap boards, 10 markers, 24 eraser pads** in
three boxes. No carry handle.

This is a variant of the original whiteboard caddy
([`../../whiteboard-stand/`](../../whiteboard-stand/)): a third eraser bay was
added and the carry handle was removed.

Generated parametrically by [`build_caddy.py`](build_caddy.py) in FreeCAD.
Checked (fit/printability/mesh) but not yet printed. Licensed MIT — see
[LICENSE](../../LICENSE);
...
| Erasers | **3 boxes**, 8 pads each = 24 pads (50 × 50 × 8 mm) |
| Box size | 58 × 58 × 73 mm each, side by side, centred |
| Handle | none — both walls are plain |
...
- **Structural** — bay post width, web between adjacent marker bores.
...
Current state: **no flat ceilings, no curved overhang**, body or coupon; both
meshes watertight.
...
- **No carry handle.** This variant drops the arched cutout entirely — both
  walls are plain, uncut slabs.
...
- **The three eraser boxes block front access to eight of the ten markers,
  leaving only the two outermost clear.** The boxes are 73 mm tall and the
  tubes 50 mm, so those markers are reached from above rather than head-on.
  Markers stand ~85 mm proud of the tube rim, so this is awkward-looking
  rather than actually obstructive. Worse than the two-box original — if
  front access matters more than a third eraser bay, reduce `pads_per_bay`
  instead to keep the row narrower.
