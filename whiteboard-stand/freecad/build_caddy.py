...
PARAMS = {
    ...
    # --- erasers ----------------------------------------------------------
    "n_bays":         2,       # two separate eraser compartments
    "pads_per_bay":   8,       # each box holds a full stack, as originally
                               # designed - so 16 pads total
    "pad_w":          50.0,
    "pad_d":          50.0,
    "pad_t":          8.0,
    "bay_wall":       2.4,
    ...
    # --- handle -----------------------------------------------------------
    "handle_w":       80.0,    # arch span; the top is a semicircle of
                               # handle_w/2, so there is no flat ceiling to
                               # bridge. Fingers hook the inside of the arch.
    "handle_z":       35.0,    # arch springing line, above the base top
    "handle_top_min": 10.0,    # material that must remain above the slot
...
def derive(p):
    ...
    d["warnings"] = []
    d["handle_h"] = p["handle_w"] / 2.0        # semicircular arch rise
    above = p["wall_h"] - (p["handle_z"] + d["handle_h"])
    d["handle_top"] = above
    if above < p["handle_top_min"]:
        d["warnings"].append(
            "only %.1fmm above handle slot (want >= %.1f)"
            % (above, p["handle_top_min"]))
...
def make_board_module(d):
    """Board slot block: outer prism, leaning cavity, ribs, handle cutout.
    ...
    # Handle cutout in the back wall. A rectangular slot leaves a flat
    # ceiling the printer has to bridge across the full span; an arch does
    # not, and carries load better into the bargain.
    r = p["handle_w"] / 2.0
    hz = z0 + p["handle_z"]
    cut_y0 = y0 - 5.0
    cut_len = p["back_wall"] + 10.0
    arch = Part.makeCylinder(r, cut_len, App.Vector(0, cut_y0, hz),
                             App.Vector(0, 1, 0))
    # keep only the upper half, giving a flat sill and a semicircular head
    arch = arch.common(box(2 * r + 4, cut_len, r + 4, -r - 2, cut_y0, hz))
    blk = blk.cut(arch)
...
def run():
    ...
    if d["warnings"]:
        ...
        report.append("structural checks: clear (handle_top=%.1f "
                      "bay_post=%.1f bore_web=%.1f)"
                      % (d["handle_top"], d["bay_post_w"],
                         d["tube_pitch"] - PARAMS["tube_id"]))
